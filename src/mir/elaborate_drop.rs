    ...
    'tcx: 'b,
{
        ...
        let drop_arg = if call_destructor_only {
            ...
            let addr = Rvalue::RawPtr(
                ...
                pin_obj_place.project_deeper(
                    &[
                        ProjectionElem::Field(FieldIdx::ZERO, unwrap_ty),
    ...
    fn drop_loop_trio_for_slice(&mut self, ety: Ty<'tcx>) -> BasicBlock {
        ...
        let dropline = self
            .dropline
            .map(|dropline| self.drop_loop(dropline, cur, len, ety, unwind, None));
    ...
    fn assign(&self, lhs: Place<'tcx>, rhs: Rvalue<'tcx>) -> Statement<'tcx> {
        Statement::new(
            self.source_info,
            StatementKind::Assign(Box::new((lhs, rhs))),
        )
    }
}
